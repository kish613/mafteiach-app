...
export const getOpenAITextResponse = async (messages: AIMessage[], options?: AIRequestOptions): Promise<AIResponse> => {
  ...
  const requestedModel = options?.model || "gpt-5";
  ...
  try {
    ...
    // If model unsupported, fallback to gpt-4o once
    const msg = String(error?.message || "").toLowerCase();
    const code = error?.status || error?.code || "";
    if (requestedModel !== "gpt-4o" && (msg.includes("model") && msg.includes("not") && (msg.includes("found") || msg.includes("exist")) || code === 404)) {
